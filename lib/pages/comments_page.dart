...
class _CommentsPageState extends State<CommentsPage> {
  ...
  Future _handlePostComment(int? id) async {
    ...
    if (textFieldCtrl.text.isEmpty) {
      openSnacbar(context, "Koment tidak boleh kosong!");
    } else {
        ...
        if (hasInternet!) {
          ...
          await WordPressService()
            ...
            if (isSuccesfull) {
              ...
              openDialog(
                  context, 'Komentar sudah dikirm'.tr(), 'Terimakasih'.tr());
            } else {
              setState(() => _isSomethingChanging = false);
              openDialog(context, 'Posting error!', 'Please try again');
            }
          });
        }
      });
                        ...
                        default:
                          if (snapshot.hasError || snapshot.data == null) {
                            return EmptyPageWithIcon(
                              icon: Icons.error,
                              title: 'Error on getting data comment',
                            );
                          } else if (snapshot.data.isEmpty) {
                            return EmptyPageWithImage(
                              image: Config.commentImage,
                              title: 'Belum ada komentar'.tr(),
                              description: 'Silahkan berkomentar'.tr(),
                            );
                          }
